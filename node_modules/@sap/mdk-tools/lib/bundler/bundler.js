...
class Bundler {
  ...
  _assemble() {
    ...
    return instance._assembleApplication(instance.sBaseApplicationName, sSourcePath, instance.buildOutputPath)
      ...
      .then(instance._updateCIMLessStyleFiles)
          ...
          rules: [
            {
              ...
              use: [
                ...
                },
                {
                  ...
                  options: {
                    baseApplicationName: instance.sBaseApplicationName,
 ...
 */
  _isValidBundleFile(sFilePath) {
    ...
    const prefixPath = path.join(instance.specifiedOutputPath,instance.buildOutput);
    const relativePath = sFilePath.startsWith(prefixPath) ? sFilePath.substring(prefixPath.length+1) : sFilePath;
    if (relativePath.split(path.sep).some(sPathComponent => {
  ...
  _updateCIMLessStyleFiles() {
    // read the components for this application
    return instance._readComponentIntegrationFiles(instance.clientDefinitionsPath).then((aComponents) => {
      for (let oComponent of aComponents) {
        oComponent.IntegrationPoints = oComponent.IntegrationPoints ? oComponent.IntegrationPoints : [];
        for (let integrationPoint of oComponent.IntegrationPoints) {
          if (integrationPoint.Target.endsWith('.less')) {
            instance._updateCIMLessStyleFile(oComponent, integrationPoint);
          }
        }
      }
    });
  ...
  _updateCIMLessStyleFile(oComponent, integrationPoint) {
    const aSourceSegements = integrationPoint.Source.split('/');
    const aTargetSegements = integrationPoint.Target.split('/');

    const sApplicationName = instance.sBaseApplicationName;

    if (aSourceSegements[1] === oComponent.ApplicationName && aTargetSegements[1] === sApplicationName) {
      aSourceSegements.shift();
      aTargetSegements.shift();

      instance._replaceCIMLessStyleFile(oComponent.__path__, path.join(instance.buildOutputPath, aSourceSegements.join(path.sep)), path.join(instance.buildOutputPath, aTargetSegements.join(path.sep)), integrationPoint);
    }
    ...
    if (aSourceSegements[1] === sApplicationName && aTargetSegements[1] === oComponent.ApplicationName) {
      ...
      instance._replaceCIMLessStyleFile(oComponent.__path__, path.join(instance.buildOutputPath, aTargetSegements.join(path.sep)), path.join(instance.buildOutputPath, aSourceSegements.join(path.sep)), integrationPoint);
    }
  }

  _replaceCIMLessStyleFile(sCIMPath, sSourcePath, sTargetPath, integrationPoint) {
    console.log(logger.getPrefix('MDK Bundler - ') + logger.getInfo('processed component style src: ') + logger.getInfoMore(sSourcePath) + logger.getInfo(' dest: ') + logger.getInfoMore(sTargetPath));
    if (!fs.existsSync(sSourcePath)) {
      throw new Error(`Cannot find source path: "${sSourcePath}" in "${sCIMPath}"`);
    } else if (!fs.existsSync(sTargetPath)) {
      throw new Error(`Cannot find target path: "${sTargetPath}" in "${sCIMPath}"`);
    ...
    if (sSourcePath !== sTargetPath) {
      if (shelljs) {
        shelljs.cp('-f', sSourcePath, sTargetPath);
      } else {
        exec(`cp -f "${sSourcePath}" "${sTargetPath}"`);
      }
    }
  }
