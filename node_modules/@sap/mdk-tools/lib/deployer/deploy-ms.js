...
class DeployMS {
    ...
    _getProjectInfo() {
        return new Promise(function (resolve, reject) {
            ...
            if (instance.deployArgs.adminApi()) {
                instance._option["AdminAPI"] = instance.deployArgs.adminApi();
            }

            if (instance._option["AdminAPI"] && !instance._option["AdminAPI"].endsWith("/")) {
                instance._option["AdminAPI"] = instance._option["AdminAPI"] + "/";
            }
    ...
    _build() {
        return new Promise(function (resolve, reject) {
            if (instance.deployArgs.source() || instance.deployArgs.zip()) {
                resolve();
            }  else {
    ...
    async _zip() {
        return new Promise(function (resolve, reject) {
            if (instance.deployArgs.zip()) {
                resolve();
            } else if (instance.deployArgs.source()) {
                utils.logInfo(`Zipping project ...`, instance.deployArgs.marker());
                const sProjectPath = instance.deployArgs.project();
                const sAppName = utils.getAppName(sProjectPath);
                const zipOutFolder = path.join(sProjectPath, utils.BUILD_TEMP_FOLDER);
                fs.ensureDirSync(zipOutFolder);

                const  zipOutput = `${zipOutFolder+path.sep+sAppName}.zip`;
                if (fs.existsSync(zipOutput)) {
                    fs.removeSync(zipOutput);
                }

                instance.sBundleZipfile = zipOutput;

                let sMDKTools = utils.getMDKTools();
                let sCommand = `${sMDKTools} build --target source --project "${sProjectPath}" `;
                ...
                try {
                    ...
                    console.error(logger.getError(`Failed to zip project source with error: ${error}`));
                    reject(error);
                }
                ...
                let definistionsVersion = utils.getAppUpdateVersion(instance.deployArgs.project());
    ...
    async _checkApp(checkApps) {
        ...
        return new Promise(async function (resolve, reject) {
            ...
            if (instance.deployArgs.log()) {
                console.info(logger.getInfo(JSON.stringify(req, null, "\t")));
            }
            let response;
            try {
        ...
        return new Promise(async function (resolve, reject) {
            if (!instance._option["Neo"]) {
                ...
                try {
                    ...
                    errorMsg = 'Failed to get Mobile Services with error:' + e.message;
                    console.error(logger.getError(errorMsg));
                    reject(errorMsg);
                }
                const result = instance._checkHttpResponse(response);
                if (result.pass) {      
                    instance._services = response.data;
                    if (instance._services && instance._services.length > 0) {
                        console.info(`Retrieve services successfully.`);
                        resolve();
                    } else {
                        errorMsg = 'Failed to get services, please check Mobile Services status in the sub account.';
                        reject(errorMsg);
                    }

                } else {
                    errorMsg = 'Failed to get services with server response:' + JSON.stringify(response.data);
                    console.error(logger.getError(errorMsg));
                    reject(errorMsg);
                }
            } else {
        ...
        return new Promise(async function (resolve, reject) {
            if (instance.isNewApp) {
                ...
                try {
                    ...
                    const error = `Failed to create app with error:${e.message}`;
        ...
        return new Promise(async function (resolve, reject) {
            ...
            if (fs.existsSync(sBundleZipfile)) {
                ...
                if (instance.deployArgs.neo()) {
                    ...
                    upLoadURL = `${instance._option["AdminAPI"]}app/${sAppId}/service/app-update/Admin/mdk/upload?_=${randomInt++}`;
                }
                else  {
                ...
                if (instance.deployArgs.log()) {
                    console.info(logger.getInfo(req.url));
                    console.info(logger.getInfo(JSON.stringify(req.headers, null, "\t")));
                }
                ...
                if (result.pass) {
                    ...
                    } else if (data && data.revision!==undefined) {
                        instance.iRevision = data.revision;
                    }
