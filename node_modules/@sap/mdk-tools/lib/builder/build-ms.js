...
class Builder {
    ...
    constructor(args) {
        ...
        instance.sBundleOutputFolder= instance.buildArgs.bundleZipTargetPath() ? instance.buildArgs.bundleZipTargetPath() : path.join(instance.buildArgs.project(), utils.BUILD_TEMP_FOLDER);
        instance.sBundleTempFolder = instance.buildArgs.bundleDefinitionPath() ? instance.buildArgs.bundleDefinitionPath() : path.join("out", path.basename(this.buildArgs.project()));
    }
    /**
    ...
    _bundleFile() {
        return new Promise(function (resolve, reject) {
            if (fs.existsSync(instance.sBundleOutputFolder)) {
                fs.removeSync(instance.sBundleOutputFolder);
            }

            //synch _SchemaVersion from application to project.json
            let sProjectPath = instance.buildArgs.project();
            let destCheProjectPath = path.join(sProjectPath, '.che', utils.MDK_IDENTITY_FILE);
            let srcCheProjectPath = path.join(utils.getRootPath(), 'builder', 'template', utils.MDK_IDENTITY_FILE);
            let definistionsVersion = utils.getAppUpdateVersion(sProjectPath);
            let configContent;
            if (!fs.existsSync(destCheProjectPath)) {
                configContent = fs.readFileSync(srcCheProjectPath, {
                    encoding: "utf8"
                });
                ...
                try {
                    configContent = fs.readJSONSync(destCheProjectPath, {
                        encoding: "utf8"
                    });
                    const oSetting = JSON.parse(configContent["attributes"]["sap.watt.common.setting"][0]);
                    oSetting["definitionsVersion"] = definistionsVersion;
                    configContent["attributes"]["sap.watt.common.setting"] = [JSON.stringify(oSetting)];
            ...
            let sMDKTools = utils.getMDKTools();
            let sCommand = `${sMDKTools} bundle --nativescript-project "${instance.sBundleOutputFolder}" --seam-project "${sProjectPath}" `
                + `--externals ${aExternals.join(" ")} `
                + `--filters ${aFilters.join(" ")} `
                + `--bundle-zip-target-path "${instance.sBundleOutputFolder}" `
            ...
            if (instance.buildArgs.target() == utils.BUILD_TARGET.ZIP) {
    ...
    _checkResult(output) {
        let bundleFile = "";
        if (instance.buildArgs.target() == utils.BUILD_TARGET.ZIP) {
            bundleFile = path.join(instance.sBundleOutputFolder, utils.BUNDLE_ZIP_FILE);
        } else {
            bundleFile = path.join(instance.sBundleOutputFolder, utils.BUNDLE_JS_FILE);
        }
    ...
    _getExtensionProjects() {
        ...
        const oApp = fs.readJSONSync(path.join(sProjectPath, utils.APPLICATION_FILE));
        if (oApp["_Type"] === APPLICATION_TYPE.Base) {
            ...
            cimFiles.forEach(cimFile => {
                ...
                if (cimContent["ProjectName"]) {
                    ...
                    const extProjectPath = path.join(rootPath, cimContent["ProjectName"], utils.APPLICATION_FILE);
    ...
    _applyOneMergeMap(mergeMapFile) {
        ...
        if (relativePath === "Application.mergemap") {
            targetBaseFile = path.join(instance.buildRoot, instance.baseProjectName, utils.APPLICATION_FILE);
    ...
    _getExternalsFromProject(sProjectPath) {
        ...
        let mdkSettingFile = path.join(sProjectPath, utils.MDK_CONFIG_JSON);
